...
(function install(factory) {
  ...
  // F1h maps to nth-root type 24h, EF34h maps to log-base type 28h, EF33h
  // maps to summation type 29h, and F0h maps to postfix-power type 2Ah.
  ...
  function editorInsertStructuralTemplate(input, sourceToken = [0xef,0x2e]) {
    ...
    if (renderType !== 0x20 && renderType !== 0x22 && renderType !== 0x23 &&
        renderType !== 0x24 && renderType !== 0x28 && renderType !== 0x29 &&
    ...
    const allocationCount = renderType === 0x22 || renderType === 0x29 ? 5 :
      renderType === 0x23 ? 4 :
    ...
    const integralAtCursor = leaf => {
      ...
      if (leaf && leaf.kind === 'editorCursor') {
        ...
        if (cursorIndex < 0)
          throw new RangeError(
            'integral insertion sequence has no direct cursor');
        originalCursor = leaf.parts[cursorIndex];
        left = leaf.parts.slice(0,cursorIndex).map(clone);
        right = leaf.parts.slice(cursorIndex + 1).map(clone);
      } else {
      ...
      if (!active || !Array.isArray(active.payload) ||
          ...
          originalCursor.byte_offset > active.payload.length)
        throw new RangeError(
          'integral insertion requires a valid active payload split');
      if (right.length) {
        const remainder = stripFirstPackedUnit(right[0]);
        if (remainder === null) right.shift();
        else right[0] = remainder;
      ...
      const structural = {
        kind:'integral',
        lower:child(structuralId + 1,true),
        upper:child(structuralId + 2),
        body:child(structuralId + 3),
    ...
    const summationAtCursor = leaf => {
      ...
      if (leaf && leaf.kind === 'editorCursor') {
        ...
        if (cursorIndex < 0)
          throw new RangeError(
            'summation insertion sequence has no direct cursor');
        originalCursor = leaf.parts[cursorIndex];
        left = leaf.parts.slice(0,cursorIndex).map(clone);
        right = leaf.parts.slice(cursorIndex + 1).map(clone);
      } else {
      ...
      if (!active || !Array.isArray(active.payload) ||
          ...
          originalCursor.byte_offset > active.payload.length)
        throw new RangeError(
          'summation insertion requires a valid active payload split');
      if (right.length) {
        const remainder = stripFirstPackedUnit(right[0]);
        if (remainder === null) right.shift();
        else right[0] = remainder;
      ...
      const structural = {
        kind:'summation',
        variable:child(structuralId + 1,true),
        lower:child(structuralId + 2),
        upper:child(structuralId + 3),
    ...
    const visit = value => {
      ...
      if (value.kind === 'editorCursor') {
        ...
        return unaryStructuralSpec
          ...
          : renderType === 0x29 ? summationAtCursor(value)
