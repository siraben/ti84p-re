...
main_names = read_main_names(TOOLS / "bcalls.txt")
...
valid_count = 0
with (TOOLS / "bcall_targets.txt").open("w", encoding="utf-8") as fp:
...
with (TOOLS / "bjumps.txt").open("w", encoding="utf-8") as fp:
...
with (TOOLS / "bcalls8x_targets.txt").open("w", encoding="utf-8") as fp:
    if kind == "bootfree":
        ...
        count = 0
        for id_value, name in sorted(read_boot_names(TOOLS / "ti83plus.inc").items()):
