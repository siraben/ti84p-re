...
```sh
nix develop -c python3 tools/community-loader-traces/build.py \
...
nix develop -c cc -O2 -Wall -Wextra -Werror \
  ...
  tools/community-loader-traces/scan_trace.c
```
...
"$TILEM" --headless --rom "$ROM" --model ti84p --normal-speed --reset \
  --macro tools/community-loader-traces/runcount.macro \
...
"$TILEM" --headless --rom "$ROM" --model ti84p --normal-speed --reset \
  --macro tools/launch-fixtures/run-first.macro \
...
`4030h` (`_newContext`). Both IDs already have those names in `tools/bcalls.txt`;
