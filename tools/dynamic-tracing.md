...
The sample programs in [`tools/tibasic-samples/`](tibasic-samples/) are raw
`ProgObj` token bodies generated by:
...
The current headless TilEm runner does not silently load `.8xp` files before
executing a macro. To trace these as stored programs today, load the chosen
program into a clean calculator RAM image with the editor, GUI send-file path, or
another link-file tool, then run:
...
`PRGM`, selects the first `EXEC` entry, and presses `ENTER`. For the `Asm(` smoke
test, load both `asmret` as `prgmASMRET` and the wrapper as the first executable
program. `ASMRET` contains only `AsmPrgm` plus the hex byte `C9`, so the Z80
payload returns immediately to the BASIC interpreter. The wrapper uses the
program-name token `0x5F` for the displayed `prgm` prefix.
