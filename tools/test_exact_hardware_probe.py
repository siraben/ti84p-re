...
from build_hardware_probes import PROBES, initial_probe_payload
from hardware_probe import decode_probe_frame, probe_verification_code
from run_exact_hardware_probe import (
    DISPLAYED_PROBES,
    parse_exact_output,
...
class ExactHardwareProbeTest(unittest.TestCase):
    ...
    def test_both_exact_adapters_accept_execution_frame_transitions(self) -> None:
        ...
        for source_name in ("tilem_exact_probe.c", "wabbitemu_exact_probe.cpp"):
            with self.subTest(source=source_name):
                text = (tools / source_name).read_text()
    ...
    def test_tilem_exact_adapter_continues_an_ordinary_clock_slice(self) -> None:
        text = (Path(__file__).parent / "tilem_exact_probe.c").read_text()
