...
from ti84re.paths import ROOT, TOOLS, SYMBOLS, DEFAULT_ROM


HERE = Path(__file__).resolve().parent
NAMES = SYMBOLS / "names.txt"
CASES = {
    "1FFF": True,
    "2000": True,
    "2001": False,
...
def env() -> dict[str, str]:
    """Environment for child Python processes: make ti84re importable."""
    return {**os.environ, "PYTHONPATH": os.pathsep.join(filter(None, [str(TOOLS), os.environ.get("PYTHONPATH")]))}
...
def run(cmd: list[str], *, stdout: Path | None = None) -> None:
    ...
    with stdout.open("w", encoding="utf-8") as stream:
        subprocess.run(cmd, cwd=ROOT, check=True, stdout=stream, env=env())
