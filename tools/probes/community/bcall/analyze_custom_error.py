...
from ti84re.trace.hardware import count_resolved_trace_points, iter_resolved_memory_writes
...
def main() -> None:
    ...
    counts = count_resolved_trace_points(
        args.trace, set(REQUIRED_POINTS) | {UNEXPECTED_RETURN},
    ...
    missing = [
        name for point, name in REQUIRED_POINTS.items()
        if not counts.counts.get(point)
    ]
    if missing:
        raise SystemExit(f"trace misses required points: {', '.join(missing)}")
    if counts.counts.get(UNEXPECTED_RETURN):
        raise SystemExit("_ErrCustom1 returned to the fixture unexpectedly")
    ...
    row = {
        ...
        "macro_sha256": sha256(args.macro),
        ...
        "unexpected_return_hits": counts.counts.get(UNEXPECTED_RETURN, 0),
