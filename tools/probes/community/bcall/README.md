...
in `tools/notes/dynamic-tracing.md`:

```sh
nix develop -c spasm \
  tools/probes/community/bcall/custom-error.asm \
  /tmp/community-custom-error.bin
nix develop -c python3 tools/probes/community/bcall/build.py \
...
"$TILEM" --headless --rom tools/rom.bin --model ti84p --normal-speed --reset \
  --macro tools/probes/community/bcall/custom-error.macro \
...
nix develop -c python3 \
  tools/probes/community/bcall/analyze_custom_error.py \
...
nix develop -c spasm \
  tools/probes/community/bcall/manual-valid.asm \
  "$fixture_dir/manual-valid.bin"
nix develop -c python3 tools/probes/community/bcall/build_manual.py \
  "$fixture_dir/manual-valid.bin" "$fixture_dir"

"$TILEM" --headless --rom tools/rom.bin --model ti84p --normal-speed --reset \
  --macro tools/probes/community/bcall/manual-valid.macro \
  --trace /tmp/community-manual-bcalls.trace --trace-range all \
  "$fixture_dir/AMANUAL.8xp" "$fixture_dir/MANBCALL.8xp"

nix develop -c python3 tools/probes/community/bcall/analyze_manual.py \
...
```sh
nix develop -c python3 -m ti84re.tibasic.build_asm_wrapper \
  COOL /tmp/ACOOL.8xp --wrapper-name ACOOL

"$TILEM" --headless --rom tools/rom.bin --model ti84p --normal-speed --reset \
  --macro tools/probes/community/bcall/cool-invalid.macro \
...
nix develop -c python3 \
  tools/probes/community/bcall/analyze_cool_invalid.py \
