...
nix develop --command python3 tools/probes/community/vat/build.py \
  --corpus "$COMMUNITY_ARCHIVE/mirror/pub/83plus/asm" \
  --out-dir "$fixture_dir"
```

Use the patched TilEm build documented in `tools/notes/dynamic-tracing.md`. Each
...
$TILEM --headless --rom tools/rom.bin --model ti84p --normal-speed --reset \
  --macro tools/probes/community/vat/prgmhide-archive.macro \
...
$TILEM --headless --rom /tmp/community-prgmhide-archive.rom \
  ...
  --macro tools/probes/community/vat/prgmhide-cold-reset.macro \
...
```sh
nix develop --command python3 tools/probes/community/vat/analyze.py \
