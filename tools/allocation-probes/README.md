...
nix develop -c "$SPASM" -L -E -I path/to/spasm-ng/inc \
  tools/allocation-probes/resident-allocation.asm \
  /tmp/resident-allocation.bin

nix develop -c python3 tools/allocation-probes/build.py \
  /tmp/resident-allocation.bin /tmp/resident-allocation-fixture

"$TILEM" --headless --rom tools/rom.bin --model ti84p --normal-speed \
  --reset --macro tools/allocation-probes/run.macro \
...
nix develop -c python3 tools/allocation-probes/analyze.py \
