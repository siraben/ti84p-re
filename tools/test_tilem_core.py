...
from tilem_core import TilemCoreError, build_command


class TilemCoreTests(unittest.TestCase):
    @patch("tilem_core.validate_tilem_source")
    def test_build_command_accepts_multiple_adapters(self, validate_source):
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = build_command(
                source,
                [Path("tools/tilem_probe_support.c"), Path("tools/probe.c")],
                Path("/tmp/tilem-probe"),
            )

        validate_source.assert_called_once_with(source)
        self.assertLess(
            command.index("tools/tilem_probe_support.c"),
