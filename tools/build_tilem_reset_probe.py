    ...
    try:
        ...
        report = {
            ...
            "adapters": [
                {
                    "path": str(TOOLS / name),
                    "sha256": file_sha256(TOOLS / name),
                }
                for name in ("tilem_probe_support.c", "tilem_reset_probe.c")
            ],
            "output": str(args.output),
            "output_sha256": file_sha256(args.output),
            "command": command,
        }
    except (OSError, TilemResetError) as error:
        parser.error(str(error))

    if args.json:
        print(json.dumps(report, indent=2))
    else:
        print(f"built pinned TilEm {TILEM_COMMIT[:8]} reset probe: {args.output}")
