...
nix develop -c spasm -E \
  tools/community-link-probes/link-wait.asm /tmp/community-link-wait.bin
nix develop -c python3 tools/community-link-probes/build.py \
...
"$TILEM" --headless --rom tools/rom.bin --model ti84p --normal-speed --reset \
  --macro tools/community-link-probes/run-one-sided.macro \
