...
from ti84re.rom.bcall_tables import find_main_table_page, main_target, read_main_names, target_is_valid
from ti84re.rom.image import RomImage
from ti84re.paths import TOOLS, SYMBOLS, DATA, DEFAULT_ROM
...
def include_equates() -> dict[int, str]:
    ...
    for line in (SYMBOLS / "ti83plus.inc").read_text(encoding="latin-1").splitlines():
...
def target_rows() -> dict[int, tuple[str, int, int]]:
    ...
    for line in (SYMBOLS / "bcall_targets.txt").read_text(encoding="ascii").splitlines():
...
class CommunityBcallCoverageTests(unittest.TestCase):
    def test_all_24_are_official_include_equates_and_curated_rows(self) -> None:
        curated = read_main_names(SYMBOLS / "bcalls.txt")
    ...
    def test_each_table_entry_decodes_to_the_committed_valid_target(self) -> None:
        ...
        names = read_main_names(SYMBOLS / "bcalls.txt")
    ...
    def test_every_row_has_a_behavior_class_and_existing_documentation(self) -> None:
        with (DATA / "community-bcall-behavior-coverage.csv").open(
        ...
        for bcall_id, row in by_id.items():
            ...
            if row["evidence_file"]:
                self.assertTrue((DATA / row["evidence_file"]).is_file())
