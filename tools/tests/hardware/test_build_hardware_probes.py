...
from ti84re.tifiles.program import asmprgm_body
...
class HardwareProbeBuilderTests(unittest.TestCase):
    ...
    def test_default_lcd_probe_rejects_hidden_columns_and_restores_after_status(self):
        text = (
            PROBE_SOURCES / "hardware" / "lcd-controller.asm"
        ).read_text()
    ...
    def test_raw_battery_probe_rejects_status_writeback(self):
        ...
        saved_status_address = USER_MEM + len(machine_code) + 3 - PROBES["battery-raw"].payload_size
        machine_code = machine_code.replace(
            bytes.fromhex("D304D304"),
            bytes.fromhex("3A") + saved_status_address.to_bytes(2, "little")
            + bytes.fromhex("D304D304")
        )
