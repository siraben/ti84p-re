...
from ti84re.hardware.compact_probe_code import decode_compact_probe_code
...
class HardwareProbeTests(unittest.TestCase):
    ...
    def test_battery_cleanup_does_not_compare_interrupt_status(self):
        for probe_id in (6, 7):
            ...
            restored = bytes.fromhex("09F08020" if probe_id == 6 else "06F08020")
            outcome = b"" if probe_id == 6 else bytes((0,))
            frame = ProbeFrame(
                probe_id=probe_id, asic_id=0x45, status=0xE3,
                payload=pre + outcome + bytes(16) + bytes(5) + restored + bytes((0xE3,)),
            )
            report = decode_probe_measurements(frame)
            self.assertTrue(report["cleanup_matches"])
            if probe_id == 6:
                ...
                self.assertEqual("0x06", report["restored"]["port_0x04_write"])
