...
class HardwareProbeBuilderTests(unittest.TestCase):
    ...
    def test_ram_alias_persists_originals_before_mutation(self):
        text = (
            Path(__file__).parent / "hardware-probes" / "ram-alias.asm"
        ).read_text()
    ...
    def test_execution_probe_uses_readable_mapping_context_not_port04_status(self):
        text = (
            Path(__file__).parent / "hardware-probes" / "execution-fetch.asm"
        ).read_text()
    ...
    def test_rtc_probe_has_progress_and_rollover_watchdogs(self):
        text = (
            Path(__file__).parent / "hardware-probes" / "rtc-rollover.asm"
        ).read_text()
    ...
    def test_raw_battery_probe_guards_context_before_sampling(self):
        text = (
            Path(__file__).parent / "hardware-probes" / "battery-raw.asm"
        ).read_text()
    ...
    def test_interrupt_probe_skips_restore_writes_on_untouched_abort(self):
        text = (
            Path(__file__).parent / "hardware-probes" / "interrupt-halt.asm"
        ).read_text()
    ...
    def test_keypad_operator_waits_are_bounded(self):
        text = (
            Path(__file__).parent / "hardware-probes" / "keypad-settle.asm"
        ).read_text()
