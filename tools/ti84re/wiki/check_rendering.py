...
from ti84re.wiki import check_katex_output as KATEX_CHECKER
...
def main() -> int:
    if len(sys.argv) not in (3, 4):
        print(
            "usage: python3 -m ti84re.wiki.check_rendering DOCS_DIR OUT_DIR [CHROMIUM_BIN]",
            file=sys.stderr,
        )
        return 2
    ...
    if not chromium:
        print("check_rendering: chromium executable not found", file=sys.stderr)
        return 2
