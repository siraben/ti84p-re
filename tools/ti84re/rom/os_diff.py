...
Pipeline (companion to tools/ghidra/studies/DumpBcalls.java):
...
Usage:
  # 1. decode + align only:
  python3 -m ti84re.rom.os_diff decode-bin  TI84Plus_OS255.8Xu  TI84Plus_OS255.bin
  # 2. decode + align + bcall-table diff against an existing raw ROM:
  python3 -m ti84re.rom.os_diff decode  OLD.8Xu  rom.bin
  ...
  python3 -m ti84re.rom.os_diff mnem  old/mnem.txt  new/mnem.txt
