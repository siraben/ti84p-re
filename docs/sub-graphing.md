...
`0x4000`). Confidence: **[confirmed]** = read the code/data directly; **[strong]** = code
plus cross-checked equate/convention; **[standard]** = matches documented TI behavior but
not byte-verified here.
...
**`_GrBufClr`** (`04:6071`): clears the whole 0x300-byte buffer to 0 (a simple
...
draw-mode (set/clear/xor) are passed in. **`_DarkLine`** (`04:4025`) is just `_ILine` with
...
pixel-aspect via `deltaX`/`deltaY` so a `Circle(` looks round only after a ZSquare. [strong]
...
re-plot and just `_GrBufCpy` the existing buffer when nothing changed. [strong]
...
higher Xres skips columns (faster, coarser). [strong]
...
- **TRACE** moves a cursor along a selected function: it steps the column, evaluates the
  ...
  X/Y it prints at the bottom. [strong]
