...
- **Command bytes** (all grounded in `_ClrLCDFull`/`_ClearRow`/`lcd_set_col_cmd`):
  - **Row (page) select** = `0xB8 + row`, sent to `0x10` via `lcd_set_col_cmd` (`01:5A89`), which only emits the byte when `0x80 ≤ A < 0xC0` (guards the row/Z-address range).
  ...
  - **Contrast**: `lcd_set_contrast` (`01:5A59`) writes the contrast level to the **data** port `0x11`; the level is held in RAM at `0x8451` and re-sent as a command via `lcd_set_col_cmd` on read-back (`lcd_get_contrast`, `01:5A60`). **[confirmed]**
