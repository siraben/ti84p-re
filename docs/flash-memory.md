...
The lower-bound bcall, its register contract, and its executable example are
in [Flash bcall programming guide](flash-bcall-guide.md#_setflashlowerbound-bcall).
...
[Flash emulator comparison](flash-emulator-comparison.md) records where the
four inspected emulators agree with the ROM's command bytes and sector
boundaries, and where they diverge on illegal transitions, completion
timing, status reads, and ASIC access control.
