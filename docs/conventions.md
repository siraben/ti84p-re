...
- **The Ghidra database** is rebuilt from the ROM by `tools/build.sh` (a 10-stage headless pipeline). It loads all 64 flash pages (page 0 + overlays at `4000`), then resolves and names routines from the main OS and retail boot tables.
- **bcall table resolution.** The main jump table page was found by *scoring* all 64 flash pages: for each candidate, count how many of the known bcall IDs produce a valid `(addr, page)` entry. Page 0x3B scored 447/535 (the runner-up only 124) for the `0x4xxx` table and is cross-checked against the documented RST shortcuts (all six matched). `0x8xxx` names come from the full SDK equates and resolve through the retail boot table on page `3F`; USB boot entries point into page `2F`. The resolver refuses to emit these body targets from a BootFree-substituted page `3F`.
