...
## How code uses it
- bcalls to a paged body set `port_mapBankA` (port 6) to the target routine's page, run it at `4000+`, then restore the previous page; bcalls whose body lives on page 0 run it in place below `4000` (e.g. `_JErrorNo`→`00:2799`). See [03-bcall-mechanism.md](03-bcall-mechanism.md). The helper `ram:181c` is the page-set primitive used by the dispatcher.
