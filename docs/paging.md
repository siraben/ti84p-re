...
`0000–3FFF` is hardwired to flash page 0. `C000–FFFF` is RAM in the static OS model and is normally the stack/user-RAM window; the 84+ hardware banks the high RAM slot through MemC/port 5. The dynamic trace in [RAM pages](ram-pages.md) confirms port-5 restores to RAM page `80` during normal OS execution. [confirmed]

## How code uses it
- bcalls to a paged body set `port_mapBankA` (port 6) to the target routine's page, run it at `4000+`, then restore the previous page; bcalls whose body lives on page 0 run it in place below `4000` (e.g. `_JErrorNo`→`00:2799`). See [bcall-mechanism.md](bcall-mechanism.md). The helper `ram:181c` is the page-set primitive used by the dispatcher.
