...
> **Correction** (from the TI-BASIC subagent deep dive, `sub-tibasic.md`): the region at `page_38:4000` is **executable dispatch code** (sequences like `CALL 0x33AB; …`), not a flat array of 2-byte handler pointers — the earlier `ParserTable.java` heuristic (runs of `0x4000–0x7FFF` words) mis-read 3-byte instructions as pointers. The dispatch is still table-like in effect, but it's code. See `sub-tibasic.md` for the byte-verified execution model (`eval_stmt_entry`@`38:59C5`, the `blockmatch_end_else`@`38:4130` End/Else matcher, `goto_lbl_name_scanner`@`38:4870`).
