...
summary `tools/fixtures/exact-hardware-probe-matrix.json` contains every
...
The compact-display record `tools/fixtures/compact-probe-e2e.json` runs the
...
`tools/fixtures/compact-probe-link-e2e.json` repeats the test with the 276-byte
...
```sh
python tools/run_compact_probe_e2e.py \
  --check tools/fixtures/compact-probe-e2e.json
python tools/run_compact_probe_e2e.py \
  --check tools/fixtures/compact-probe-link-e2e.json
```

The deterministic `HWPMAP` record is
`tools/fixtures/mapper-overlays-emulators.json`. It binds the two exact runs to
...
The exact `HWPLCD` rows use the same 1,319-byte image. Its SHA-256 is
...
MAME 0.287 completed only `tools/run_mame_lcd_probe.py`. That adapter drives
...
```sh
nix develop -c python tools/build_tilem_exact_probe.py \
  --source /path/to/tilem-f56ad637 --output /tmp/tilem-exact

nix develop -c python tools/build_wabbitemu_exact_probe.py \
  --source /path/to/wabbitemu-48c2dc0 --output /tmp/wabbitemu-exact

nix develop -c python tools/run_exact_hardware_probe.py \
...
```sh
nix develop -c python tools/run_exact_hardware_probe_matrix.py \
  --backend tilem --binary /tmp/tilem-exact \
  --expected-binary-sha256 \
    ac280251dcda1cda083196abf88032502420351732cb689cac38d20348126408 \
...
nix develop -c python tools/run_wabbitemu_prefix_m1_probe.py \
  --binary /path/to/wabbitemu-headless \
  --expected-binary-sha256 \
    3acb6a18280f9c42d6fe324188eab73f87280ee70b973e1251fcfa50f54fb14e \
